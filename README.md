...
// 神煞表结构化（卷一：歲/十天干/十二地支三表映射 + 月令杂列保守解析）
import { getShenShaSections, shenShaValue } from 'lrdq-ts-lib/shensha';
...
|---|---|---|
| 序 | 四庫全書總目提要 | 整卷典籍化 |
| 卷一 | 起例（入手法/九宗门口诀/神煞表） | 整卷典籍化 + 神煞**深度结构化**（`shensha.json`：三表映射 + 月令杂列保守解析；總鈐盘图按红线列存不重构） |
