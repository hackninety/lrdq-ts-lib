...
```bash
npm install github:hackninety/lrdq-ts-lib#v0.11.0
```
...
// 类神（卷二《神將釋》：12 月将「所主/類為/為姓」要点 + 12 天将定性段，論/賦/詩/類分块）
import { findTianJiang, findYueJiang } from 'lrdq-ts-lib/leishen';

findTianJiang('青龙'); // 简繁/古写归一 → 青龍論全块 + brief 定性段
findYueJiang('亥'); // 登明亥（所主/類為/為姓原行）

// 占例库（《指南注解》卷三 135 例：占驗＝陳公獻原案、占例/增補＝張洪今案）
import { getZhanLi, findSimilarCases, zhanLiLabel } from 'lrdq-ts-lib/cases';

findSimilarCases({ keti: ['蒿矢'], chapters: ['疾病章第九'], day: '乙未', limit: 2 });
// → [{ entry, score, why: ['同章「疾病章第九」', '课体同「蒿矢」', …] }]
// 头部字段（年/月/日干支/时支/月将/课体标签/旬空）保守解析；案文照录含课式盘图；
// 底本旬空与日干支机器互证（117 例互证、1 例干支讹照录记校记）
```
...
卷三會纂占驗指南（總論~諸占 32 章分章 + **占例深度结构化**：`cases.json` 135 例，
编号占驗/占例连同增補課例分块、头部字段保守解析、旬空互证）+ 卷之四神煞指南。
陳注與張注底本混排照录；今注版权属注者，非商用汇编、异议即撤（见 LICENSE）。
