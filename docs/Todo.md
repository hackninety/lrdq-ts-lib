...
| **大六壬指南（壬归）** | 明末清初 · 陈公献 | 占验派最重要，分类占例极多 | ✅ v0.8.0 以《六壬指南注解》收录（slug=lrzn，ctext res=516644，張洪 2000 注解本；红线撤销后按非商用汇编口径收，署名"陳公獻撰·張洪注"，provenance/LICENSE 声明异议即撤）。**年代考（勿再误信"清代张洪"说，百度百科有此讹）**：自序"庚戌季秋以百七十元貿得上海民國江東書局《改良大六壬全書三種》，三十年間苦讀"、落款"庚辰年秋"＝1970 购民国旧书→2000 作序；"觀今者納甲星命充棟汗牛"为当代出版生态；卷首語引今人"季銀國先師"；历代书目无清张洪注本著录 | ctext |

### T1.5 · 指南注解卷三占例深度结构化（✅ v0.11.0）

`cases.json` + `lrdq-ts-lib/cases` 子入口：卷三 135 例全量结构化——「占驗N、」＝陳公獻原案 125、
「占例N、」与「增補課例/占例：」标记后诸案＝張洪今案 10（`augmented` 标志，含 1 例无编号今案）。
头部字段（年/月/日干支/时支/月将/课体标签/旬空/落空）保守解析、失配留空；案文照录含课式盘图原行。
**旬空金标互证**：117 例日干支↔旬空机器互证，仕宦章占驗十五「辛亥日…子丑空」干支有讹照录记校记。
`findSimilarCases({keti, chapters, day, kong})` 同章 +4／课体 +3（封顶 6）／同日干支 +2／同旬空 +1，
供宿主把相似占例作 few-shot 附进 AI 断课 Prompt（占事定向章名与 `entry.chapter` 同源对齐）。
...
## 4. 数据源与版权（2026-07-06 修订：撤销"勿收在版权期著作"红线）

- **本库许可**：v0.8.0 起整库改 **CC BY-NC 4.0（禁止商用）**，见 LICENSE
- **收录口径**（用户拍板）：以非商业研习汇编为目的，可收在版权期的当代注解/整理内容，
  但必须——①如实署名注者与年代；②底本行标注出处（res/url）；③LICENSE 与该书 provenance
  声明"权利人异议即撤"（takedown on request）；④无法确认作者/出处的内容仍不收
- **公有领域**：清及以前无虞；民国印本须作者殁逾 50 年（袁树珊 1881–1968，已过，可收）
